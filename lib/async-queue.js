export default class {
  ...
  constructor(options = {}){
    ...
    this.#last     = new function(){
                           ...
                           this.state   = "pending";
                           this.staged  = true;
                           this.promise.catch(_ => {}); // to prevent uncaught error at stage when the queue is enqueued but never consumed.
                         }();
    this.#size     = 0;
    this.#live     = true;
    this.#events   = { "next" : []
    ...
    this.#Link     = class {
                       ...
                       constructor(item){
                         ...
                         this.promise  = new Promise((resolve,reject) => {
                                                       AQ.timeout && setTimeout( e => this.previous.state === "pending" && ( this.previous.state = "aborted"
                                                       ...
                                                       this.reject  = reject;
                                                       Object(item).constructor === Promise ? item.then(v  => this.previous.state !== "aborted" && ( this.previous.state = "resolved"
                                                                                                                                                     , this.previous.resolve(v)
                                                                                                                                                     , AQ.#runEvent("reply", { value: v
                                                                                                                                                                             , pid  : this.id
                                                                                                                                                                             })
                                                                                                                                                     , AQ.raceMode && AQ.clear("upto",this.id)
                                                                                                                                                     ))
                                                                                                  .catch(e => ( (typeof e !== "object" || e === null) && (e = { name   : "Anonymous"
                                                                                                                                                              , message: e
                                                                                                                                                              })
                                                                                                              , e.pid = this.id
                       ...
                       get staged(){
                         return AQ.#stage && (this.promise === AQ.#stage.promise);
                       }
      ...
      try {
        if (this.#live) {
          !handled && ( yield this.#stage.promise
                      , this.#runEvent("next", this.#head.id)
                      )
        }
        else return void 0;
      }
